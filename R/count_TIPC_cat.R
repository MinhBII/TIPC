...
#' res <- count_TIPC_cat(root_dir=root_dir)
#' @export
count_TIPC_cat <- function (root_dir = NULL){
  ...
  ## results check
  ## ======================
  if(file.exists(file.path(root_dir,'TIPC_counts.RData')))warning('TIPC category counts existed!\n')

  ## ======================
  ## load tessellated_list
  ## ======================
  tessellated_list_holder <- load(file.path(root_dir,'tessellated_res.RData'))
  ...
  save(TIPC_counts, file = file.path(root_dir,'TIPC_counts.RData'))
  return(TIPC_counts)
