...
#' res <- tessellation(cell_data = cell_data, output_dir = output_dir)
#'
#' @export
...
tessellation <- function(cell_data = NULL, hex_len = 100,
  ...
  if(!'immune' %in% all_celltypes) stop('No immune cells found in input cell data!\n')
  if(!'tumor' %in% all_celltypes) stop('No tumor cells found in input cell data!\n')
  if(!'stroma' %in% all_celltypes) stop('No stromal cells found in input cell data!\n')
  ...
  ## results check
  ## ======================
  if(file.exists(file.path(output_dir,'tessellated_res.RData')))warning('Tessellated results existed!\n')
  ...
  setwd(output_dir)
  res_list <- list(M_tumor_list_O=M_tumor_list,
  ...
  save(res_list, file = 'tessellated_res.RData')
  return(res_list)
