...
trend_plot_hexLen <- function(root_dir =  NULL, hex_len_range = NULL, hex_len_stepsize =  NULL){
  ...
  ## ======================
  set.seed(123)
  hexLen_col_vec <- distinctColorPalette(length(subfolder_nms))
  dir_col_vec <- distinctColorPalette(5)
  ...
  for (f_nm in subfolder_nms){
    ...
    ## load TIPC_counts
    ## ---------------
    TIPC_counts_holder <- load(file.path(f_nm,'TIPC_counts.RData'))
  ...
  plot_fnm <- paste0('TIPCmetric_vs_hexLen',paste0(all_hexLen, collapse = '_'), '_percentage.pdf')
  plot_fnm <- file.path(root_dir, plot_fnm)
  dodge <- position_dodge(width = 0.9)
  ggsave(
